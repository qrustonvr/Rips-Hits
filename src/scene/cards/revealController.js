...
//
// One `t`-style state machine per card walks the PLAN.md beat sequence:
//   slide-in → pre-flip glow (held longer the rarer it is) → input gate
//   → flip (reveal effects fire at 90°) → hold (price fades in) → dismiss.
//
// 3D side (cards, particles, camera shake, screen flash) lives here. UI text
// (prompts, name/rarity, price, summary) is emitted as window CustomEvents so
// the Open screen owns presentation. Everything reads the rarity table, so
// pacing/feel is tuned in one place (game/rarity.js).
...
export class RevealController {
  ...
  _onUp(e) {
    ...
    if (dist > 70) this._onSwipe();          // a real drag
    else if (dt < 500) this._onTap();          // a quick tap
  }
  ...
  _fireReveal() {
    ...
    this._flash(r.flash);
    this._haptic(r.tier);
    if (r.glow === 'rainbow-cycle') this.shake = 0.16;     // Secret Rare shake
  ...
  _finish() {
    ...
    // best = highest-rank card pulled
    const best = this.cards.reduce(
      (a, b) => (!a || rank(b.rarity.tier) > rank(a.rarity.tier) ? b : a),
      null
    );
    window.dispatchEvent(new CustomEvent('reveal:summary', {
      detail: {
        ...
        best: best ? { name: best.data.name, label: best.rarity.label, colorCss: best.rarity.colorCss } : null,
      },
    }));
  }
  ...
  // ----- main update ---------------------------------------------------------
  update(dt, time) {
    // camera shake decay (applied every frame so it self-restores)
    ...
    switch (this.phase) {
      case P.INTRO: {
        // Let the pop/recoil settle, then sink the pack and bring up card 1.
      ...
      case P.GATE: {
        // hold glowing, wait for tap (handled in _onTap)
        if (c.rarity.glow !== 'none') {
      ...
      case P.FLIP: {
        ...
        const k = Math.min(this.timer / dur, 1);
        const e = easeInOut(k);
        c.flip.rotation.y = Math.PI * (1 - e);     // PI (back) → 0 (front)
      ...
      case P.HOLD: {
        // ramp holo intensity in over the first 0.5s
        const ramp = Math.min(this.timer / 0.5, 1);
        c.setHoloIntensity(c.rarity.holoIntensity * ramp);
        // living wobble — drives holo tilt + a little card sway
        ...
        // fade lingering glow down for non-hits
        if (!isHit(c.rarity.tier)) c.setGlow(Math.max(0, c.glow.material.opacity - dt));

        if (!this._priceFired && this.timer >= 1.5) {
          this._priceFired = true;
          window.dispatchEvent(new CustomEvent('reveal:price', {
            detail: { price: this.card.data.price },
          }));
        }
